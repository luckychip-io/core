...
contract OraclePancake is Ownable, IOracle {
    ...
    constructor(address _factory, address _anchorToken, address _USDT) public {
        factory = _factory;
    ...
    function getQuantityUSDT(address token, uint256 amount) public override view returns (uint256 quantity) {
        if (token == USDT) {
            quantity = amount;
        } else {
            uint256 anchorQuantity = getQuantity(token, amount);
            quantity = consult(anchorToken, anchorQuantity, USDT);
        }
    }
