...
contract DiceReferral is IDiceReferral, Ownable, ReentrancyGuard {
    ...
    function claimCommission() public override nonReentrant {
        ...
        if(info.pendingCommission > 0){
            ...
            token.safeTransfer(referrer, tmpAmount);
    ...
    // Owner can withdraw other tokens.
    function withdrawOtherToken(address tokenAddress) external onlyOwner {
        require(tokenAddress != address(token), "No referral basic token");
